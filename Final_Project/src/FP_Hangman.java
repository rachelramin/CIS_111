...
import java.util.Random;//need to select a random number to pick the word.
...
{ //remember to close after program is done.      
	...
	//variables that will work throughout the code, regardless of the method it is called in.
   
   //boolean that checks in the main while loop of the code.
	private static boolean exit = false;
   //incrementer that determines what stage of the code you are on.
	private static int stage = 0;
	
   //Tracks your amount of tries you have left.
   ...
   //String for if you win or lose the game.
	private static String gameWin = "Congratulations you won the game!";
	private static String gameLose = "You have run out of tries.  \nThe word was:  ";
	
   //String that saves the selected word.
	private static String word = "";
   //The character that hides the letters in the word form the user.
	private static String maskCharacter = "*";
   //The boolean array that triggers the switch from maskCharacter to letter.
   ...
   private static boolean done = false;
	...
	{	//main method open
      //While exit is not true ( exit == false )	
		while(!exit)
		{
         ...
         //word select stage.
			...
			{
				// Calls to the word selection method, which will return the string "Word".
				WordSelect();
            //increments stage.
				stage++;
			}
      ...
      //Asks the user how they would like to play, and increments the stage after the selection.
		{
			...
			else if ( !mode.trim().equalsIgnoreCase("easy") && !mode.trim().equalsIgnoreCase("hard") )
			{
				...
				JOptionPane.showMessageDialog(null,"That is not a valid mode.");            
			}  
		}
		
		public static void PlayGame()
      //game play code that calls to the displayword method to display output, asks user for a guess, and tests the guess.
		{  
         ...
         //calls to testguess while passing the input to test the inputted guess.
			TestGuess(input);
		}
      ...
      //Asks the user if they want to play he game. 
		{
         ...
         //Ask the user if they want to replay.
			...
			if(replay.trim().equalsIgnoreCase("no") )
			{
				...
				JOptionPane.showMessageDialog(null,"Thank you for playing." + 
                                                "\nYour win and lose numbers are stored in the text file WinLose.txt");
                                                
            //try-catch block used to handle exception that comes with creating, writing and closing a text file in method.                                    
            try
            {                                               
            StoreWinLooses();
            }catch (IOException ioe)
         ...
         //resets stage to 0 and resets saved guesses to blank
			else
			{
				stage = 0;
				guesses = "";
        ...
        {         
			...
			//while the txt file has lines, the loop will continue to read the lines into the string array.
			...
			//selects the random number from array range.
			int number = randomNumber.nextInt(counter);

			//stores the word stored in the random array address into a string. 
			word = words[number];
      ...
      //method determines if the display needs an asterisk or a letter.
		{
         //char array that gets rewritten every time the method is run. 
         ...
         //for loop that runs for as long as the word is.
            ...
            // if boolean [] worMask at i is not true
               ...
               //is a String).
				...
				{
               //replace the character at i with the letter at that place in the word. 
      ...
      //Method tests the input ( guess ) against previous guesses and the win/lose conditions.
		{
         ...
         //if the String guesses contains the char trash, gives error message and deducts a try.
			...
			{
				JOptionPane.showMessageDialog(null,"You already entered that letter");
         ...
         //else the guesses does not contain the char trash
			...
			{
            //concatenates that guess onto guesses string.
				guesses = guesses + guess;
            //a boolean to check win/lose.
				boolean check = false;
      ...
      //Method tests to see if user completed the word and beat the game.
		{
			for(int i = 0; i < word.trim().length(); i++)
			{
            //If the boolean array contains any falses, the user did not complete the word.
				if(wordMask[i] == false)
				{
					return false;
				}
      ...
      // allows user to edit words in txt file.
      {
         ...
         else if( options.trim().equalsIgnoreCase("a") )
         {
          ...
          outputFile.println(newWord.toLowerCase());
         ...
         {//else if 2
            ...
            //initializes a reader and writer for the parent file and new file.
            BufferedReader reader = new BufferedReader(new FileReader(inputFile2));
            BufferedWriter writer = new BufferedWriter(new FileWriter(tempFile));
   
            String line;
   
            //While loop that reads each line for the text file into the temp file
